...
def from_best_config(
    ...
    with utils.init_wandb_run(
        ...
        best_config_artifact = run.use_artifact( #karan99/energy_consumption/config:v0
            "config:v0",#"best_config:latest",
            type="model",
        )
        download_dir = best_config_artifact.download()
        config_path = Path(download_dir) / "config.json"#"best_config.json"
...
def add_best_model_to_model_registry(best_model_artifact: wandb.Artifact) -> int:
    ...
    best_model_path = Path(best_model_dir) / "best_model.pkl"#karan99/energy_consumption/config:v0
